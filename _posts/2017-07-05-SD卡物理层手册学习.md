...
读数据的时候, 以Block为单位进行读取. 可以使用多块连续读(CMD18), 或者单块读取(CMD17). 在读数据之前, 先使用CMD16来设置block的长度, 如果使用CMD16设置block长度, 长度最大为512Bytes, 则使用寄存器中的READ_BL_LEN作为块长度. 当使用CMD17进行单块读时, 读取完一个数据块, 卡将从Sending-data状态自动跳回到Trans状态. 而使用连续块读(CMD18)时, 卡将连续发送连续的block数据, 直到收到停止指令(CMD12)为止.
...
写数据与读数据类似, 也是以Block为单位的. 但是不同的是, block长度不可以小于512Bytes.
...
当写CSD寄存器的时候, 数据长度可以与block length不匹配. 有一些CSD寄存器的数据是只读的, 在写CSD的时候必须要保证卡只读部分对应的写入卡的数据必须一致. 否则写CSD寄存器会失败.
